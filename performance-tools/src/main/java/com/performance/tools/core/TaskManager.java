...
import com.performance.tools.core.activity.ActivityTask;
import com.performance.tools.core.base.ITask;
import com.performance.tools.core.block.BlockTask;
import com.performance.tools.core.fps.FpsTask;
...
public class TaskManager {
  ...
  private void startTaskSwitch(){
    List<ITask> allTask = getAllTask();
    for (ITask task : allTask){
      updataTaskSwitchByTaskName(task.getTaskName(),true);
    }
  }
  ...
  public void registTask(){
    mAllTaskMap.put(TaskConstants.TASK_ACTIVITY,new ActivityTask());
    if (Env.DEBUG){
      //性能消耗较大
      mAllTaskMap.put(TaskConstants.TASK_FPS,new FpsTask());
    }
    mAllTaskMap.put(TaskConstants.TASK_BLOCK,new BlockTask());
